...
from hackem_yandex_dialogs.core.config_manager.types.exceptions import (
    InvalidConfigTypeError,
)


class ConfigManager:
    def __init__(self, config: BaseProvider | str):
        if not isinstance(config, BaseProvider):
            self.config_path = Path(config).resolve()
            self.config_type = self.config_path.suffix.lstrip(".")
            match self.config_type:  # TODO: rewrite for automatically check available providers
                case "toml":
